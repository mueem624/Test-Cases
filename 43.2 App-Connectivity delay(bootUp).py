"""
Description:
The AIM of the test case is to verify the behavior of the DUT, when VF launcher failed to get the response from
google.com(Internet connectivity check) due to packet delay
...
try:
    for iteration in range(1, int(timesLoop) + 1):
        ...
        kmaxImpairment.switchImpairment_ON(impairment='ApplicationConnectivity', delay=6, filterNo=filterNo)
        api.waitSec(135)
        setStepStatus(api.TM.PASS)
    ...
    api.writeDebugLine("Application connectivity packet delayed and recover passed")
    setStepStatus(api.TM.PASS)
